...
- [ ] **T3** M2 交互对齐第二批（2026-08-29 更新）：斜杠命令补全（快照里的 slashCommands 数据源已就位；composer `/` 按钮插入光标已落地，剩补全下拉）、上下文用量与压缩入口（session/compact 已封装）；模型 / 模式切换器迁入 webview 已由 composer chip 完成（记录：2026-08-27 16:05）
